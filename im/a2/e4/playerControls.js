...
// Above is the button for the mute button and below is the button for the fullscrean named muteButton and fullButton respecively. these are eached named acording to their -
// - puroose as well as to make their funtion clear if this code is to be viewed by someone else or I need to easily locate either of them.
...
/* Mute/Unmute */
function muteunmute(){
  //above is the function name fittingly being named muteunmute.
  if(videoElement.muted){
    // these next two lines are running if the video is muted, displaying that the audio is playing via the button. they first check to see if the audio is playing -
    // - and if so set the icon as such.
    videoElement.muted = false;
    muteButton.style.backgroundImage = "url('./icons/mute.svg')";
  } else {
    // these two lines will only run if the audio is not playing in which case the button will change accordingly displaying that there is no audio playing. It first -
    // - makes sure that the audio i not playing and then changes the icon to show this.
    videoElement.muted = true;
    muteButton.style.backgroundImage = "url('./icons/unmute.svg')";
  }
}
muteButton.addEventListener("click", muteunmute);
//this line creates an active listerner to wait for the button to be click and then running the function when it is clicked.

/* FullScreen */ 

function Fullscreen() {
  //above is a function to make the video go fullscreen being named fullscreen.
...
//the function first checks to see if it is fullscreen. in the case that it is not, it request to become fullscreen, becoming fullscreen. if the video is already fullscreen - 
// - in then exists fullscreen, returning to the normal viewing screen.
